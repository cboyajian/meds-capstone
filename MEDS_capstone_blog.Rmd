...
![](data_viz/phase_plot.png){width=50%}

![](data_viz/timeline_plot.png){width=75%}
...
-   You will hear this plenty at Bren (if you haven't already), but **networking** is a crucial part of your grad school experience. This includes networking with your cohort! Your cohort will be some of your strongest connections after you graduate, and working on a capstone proposal is a great opportunity to build those connections. Beyond your cohort, working on a proposal with a client gives you many excellent opportunities to network with potential employers.
...
Here are some EJ themes and questions to think about while writing your proposal. These questions come from Bren's [Equity and the Environment course](https://bren.ucsb.edu/courses/esm-296-1f-0) (highly recommend taking!).

```{r}
# data frame for justice analysis questions 
justice_analysis <- data.frame(
  Number = c("1",
            "2",
            "3",
            "4",
            ...
            "15",
            "16"),
  Questions = c("Who has the power in the situation?",
                "Who has privilege?",
                ...
                "How is excessive self-centeredness playing into this - on the part of the humans?",
                "What can we do to overcome it?")
)

# turning justice_analys df into a gt object
ja_gt <- gt(justice_analysis)

# gt table 
ja_table <- ja_gt %>% 
  tab_options(column_labels.hidden = TRUE) %>% 
  tab_header(title = "Justice Analysis") %>% 
    tab_row_group(
    label = "Anthropocentrism / Species-ism",
    rows = 14:16) %>% 
  tab_row_group(
    label = "Ways of Knowing",
    rows = 11:13) %>% 
  tab_row_group(
    label = "Burden of Evidence",
    rows = 9:10) %>% 
  tab_row_group(
    label = "Racism & Xenophobia",
    rows = 7:8) %>% 
  tab_row_group(
    label = "Exclusionary Solutions",
    rows = 5:6) %>% 
  tab_row_group(
    label = "Distribution of Harm",
    rows = 4) %>% 
  tab_row_group(
    label = "Power",
    rows = 1:3)
